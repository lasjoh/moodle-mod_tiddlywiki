...
function tiddlywiki_display_embed($tiddlywiki, $cm, $course, $file) {
    ...
    if (file_mimetype_in_typegroup($mimetype, 'web_image')) {  // It's an image
        $code = tiddlywikilib_embed_image($fullurl, $title);

    } else if ($mimetype === 'application/pdf') {
        // PDF document
        $code = tiddlywikilib_embed_pdf($fullurl, $title, $clicktoopen);
    ...
    tiddlywiki_print_heading($tiddlywiki, $cm, $course);

    echo $code;

    tiddlywiki_print_intro($tiddlywiki, $cm, $course);
...
function tiddlywiki_print_workaround($tiddlywiki, $cm, $course, $file) {
    ...
    tiddlywiki_print_header($tiddlywiki, $cm, $course);
    tiddlywiki_print_heading($tiddlywiki, $cm, $course, true);
    tiddlywiki_print_intro($tiddlywiki, $cm, $course, true);
...
function tiddlywiki_print_intro($tiddlywiki, $cm, $course, $ignoresettings=false) {
    ...
    if ($ignoresettings || !empty($options['printintro']) || $extraintro) {
        $gotintro = trim(strip_tags($tiddlywiki->intro));
        if ($gotintro || $extraintro) {
            echo $OUTPUT->box_start('mod_introbox', 'tiddlywikiintro');
            if ($gotintro) {
                echo format_module_intro('tiddlywiki', $tiddlywiki, $cm->id);
            }
            echo $extraintro;
            echo $OUTPUT->box_end();
        }
