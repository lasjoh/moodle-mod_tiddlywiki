...
$sql = "SELECT id, filename, userid, timemodified, contenthash FROM {$prefix}files WHERE component = 'mod_tiddlywiki' AND filename = '{$filename}' AND contextid = '{$contextid}' AND sortorder = '1'";
...
if (mysqli_num_rows($result) == 0) {
  ...
  $oldcontent = $row['contenthash'];
if ($oldcontent === $newcontenthash) {
toExit("No changes to save");
}
...
if (!$stmt) {
    mysqli_rollback($conn);
    toExit("Prepare failed: " . mysqli_error($conn));
}

if (!mysqli_stmt_bind_param($stmt, 'issii', $newfilesize, $newcontenthash, $userid, $current_time, $id)) {
    mysqli_stmt_close($stmt);
    toExit("Failed to bind parameters for update statement: " . mysqli_error($conn));
}

if (!mysqli_stmt_execute($stmt)) {
    mysqli_stmt_close($stmt);
    toExit("Failed to execute update statement: " . mysqli_error($conn));
}
mysqli_stmt_close($stmt);
// Check for error
...
//Function to delete deprecated files in the filedir
function deleteBlob($deletecontent, $filedir_path) {
    $first_folder = substr($deletecontent, 0, 2);
    $second_folder = substr($deletecontent, 2, 2);
    $filepath = $filedir_path . '\\' . $first_folder . '\\' . $second_folder . '\\' . $deletecontent;
    ...
    if (file_exists($filepath)) {
        ...
        if (unlink($filepath)) {
            ...
            // Check if the inner folder is empty and delete it
            $inner_folder = $filedir_path . '\\' . $first_folder . '\\' . $second_folder;
            if (is_dir($inner_folder) && count(scandir($inner_folder)) == 2) {
                rmdir($inner_folder);
                echo "Inner folder deleted: " . $inner_folder . "\n";

                // Check if the outer folder is empty and delete it
                $outer_folder = $filedir_path . '\\' . $first_folder;
                if (is_dir($outer_folder) && count(scandir($outer_folder)) == 2) {
                    rmdir($outer_folder);
                    echo "Outer folder deleted: " . $outer_folder . "\n";
                }
            }
        } else {
...
if (empty($backupDir)) {
    deleteBlob($oldcontent, $filedir_path);
	toExit("No Backups, because no backup directory was set");
}
deleteBlob($oldcontent, $filedir_path);
toExit("Backups are not possible yet in this plugin version");
