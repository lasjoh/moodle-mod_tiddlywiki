...
$sql = "SELECT id, contextid, filepath, filename FROM {$prefix}files WHERE component = 'mod_tiddlywiki' AND filename = '{$filename}' AND contextid = '{$contextid}'";
...
if (mysqli_num_rows($result) == 0) {
  ...
  // Update TiddlyWiki file in Moodle files database
  $sql = "UPDATE {$prefix}files SET filearea = ?, filepath = '/', filename = ? WHERE id = ?";
  ...
  $updatedata = array('content', $filename, $id);
  if (!mysqli_stmt_bind_param($stmt, 'ssi', ...$updatedata) || 
  ...
  // Generate temp URL of updated file
  $file_path = $filepath . '/' . $filename;
  $file_url = moodle_url::make_file_url("/$contextid/mod_tiddlywiki" . $file_path);

  // Output success message, query string and temp URL
  echo 'TiddlyWiki file updated successfully.<br>';
  echo 'Query string: ' . $sql . ', parameters: ' . implode(', ', $updatedata) . '.<br>';
  echo 'Temp URL of updated file: ' . $file_url . '<br>';
}
