...
/** Legacy files not needed or new tiddlywiki */
 ...
 */
function tiddlywikilib_try_file_migration($filepath, $cmid, $courseid, $component, $filearea, $itemid) {
    $fs = get_file_storage();
 ...
 */
function tiddlywikilib_get_displayoptions(array $enabled, $current=null) {
    if (is_number($current)) {
        $enabled[] = $current;
    }

    $options = array(RESOURCELIB_DISPLAY_AUTO     => get_string('tiddlywikidisplayauto'),
                     RESOURCELIB_DISPLAY_EMBED    => get_string('tiddlywikidisplayembed'),
                     RESOURCELIB_DISPLAY_FRAME    => get_string('tiddlywikidisplayframe'),
                     RESOURCELIB_DISPLAY_NEW      => get_string('tiddlywikidisplaynew'),
                     RESOURCELIB_DISPLAY_DOWNLOAD => get_string('tiddlywikidisplaydownload'),
                     RESOURCELIB_DISPLAY_OPEN     => get_string('tiddlywikidisplayopen'),
                     RESOURCELIB_DISPLAY_POPUP    => get_string('tiddlywikidisplaypopup'));
 ...
 */
function tiddlywikilib_guess_url_mimetype($fullurl) {
    global $CFG;
 ...
 */
function tiddlywikilib_get_extension($fullurl) {
 ...
 * @return string html
 */
function tiddlywikilib_embed_image($fullurl, $title) {
    $code = '';
    $code .= '<div class="tiddlywikicontent tiddlywikiimg">';
    $code .= "<img title=\"".s(strip_tags(format_string($title)))."\" class=\"tiddlywikiimage\" src=\"$fullurl\" alt=\"\" />";
    $code .= '</div>';
 ...
 * @return string html
 */
function tiddlywikilib_embed_pdf($fullurl, $title, $clicktoopen) {
    global $CFG, $PAGE;

    $code = <<<EOT
<div class="tiddlywikicontent tiddlywikipdf">
  <object id="tiddlywikiobject" data="$fullurl" type="application/pdf" width="800" height="600">
...
EOT;
    ...
    $PAGE->requires->js_init_call('M.util.init_maximised_embed', array('tiddlywikiobject'), true);
 ...
 * @return string html
 */
function tiddlywikilib_embed_general($fullurl, $title, $clicktoopen, $mimetype) {
    global $CFG, $PAGE;
    ...
    // this is ok in HTML5.
    $code = <<<EOT
<div class="tiddlywikicontent tiddlywikigeneral">
  <iframe id="tiddlywikiobject" src="$fullurl">
    $clicktoopen
  </iframe>
</div>
EOT;
