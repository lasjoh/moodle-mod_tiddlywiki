@mod @mod_tiddlywiki @core_completion @_file_upload
Feature: View activity completion information for file tiddlywikis
  In order to have visibility of file tiddlywiki completion requirements
  As a student
  I need to be able to view my file tiddlywiki completion progress
  ...
  Background:
    ...
    And the following config values are set as admin:
      | displayoptions | 0,1,2,3,4,5,6 | tiddlywiki |
  ...
  Scenario Outline: The manual completion button will be shown on the course page for Open, In pop-up, New window and Force download display mode if Show activity completion conditions is set to No
    Given the following "activities" exist:
      ...
      | tiddlywiki | C1     | Myfile | <display> | 0        | 0        | 0        | 1          | mod/tiddlywiki/tests/fixtures/samplefile.txt | 620        | 450         | 1        |
  ...
  Scenario: The manual completion button will be shown on the activity page and course page if Show activity completion conditions is set to Yes
    Given the following "activities" exist:
      ...
      | tiddlywiki | C1     | Myfile | 1       | mod/tiddlywiki/tests/fixtures/samplefile.txt | 1        |
    And I am on "Course 1" course homepage with editing mode on
    And I navigate to "Settings" in current page administration
    And I expand all fieldsets
    And I set the field "Show activity completion conditions" to "Yes"
    And I press "Save and display"
    And I am on the "Myfile" "tiddlywiki activity editing" page
    And I set the following fields to these values:
    ...
    And the manual completion button for "Myfile" should exist
    And the manual completion button for "Myfile" should be disabled
    And I am on the "Myfile" "tiddlywiki activity" page
    And the manual completion button for "Myfile" should exist
    And the manual completion button for "Myfile" should be disabled
    # Student view.
    When I am on the "Course 1" course page logged in as student1
  ...
  Scenario: View automatic completion items
    ...
    # Student view.
    When I am on the "Myfile" "tiddlywiki activity" page logged in as student1
